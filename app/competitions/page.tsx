...
import { Calendar, Trophy, Swords, ArrowRight, Crown } from "lucide-react"
...
export default async function CompetitionsPage() {
  ...
  // Derive winners for each season
  const winnerBySeason = new Map<number, ReturnType<typeof deriveCompetitionWinner>>()
  for (const season of seasons) {
    const seasonMatches = matchesBySeason.get(season.id) ?? []
    winnerBySeason.set(season.id, deriveCompetitionWinner(seasonMatches))
  }

  // Active season winner
  const activeWinner = activeSeason ? winnerBySeason.get(activeSeason.id) : null

  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-8 lg:px-8">
      <Breadcrumbs items={[{ label: "Dashboard", href: "/" }, { label: "Competitions" }]} />
      ...
      {activeSeason && (
        <section className="mb-8 animate-fade-in-up stagger-1">
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-6">
            ...
            {/* Current leader */}
            {activeWinner && (
              <div className="mt-4 flex items-center gap-3 rounded-lg bg-card/60 px-4 py-3">
                <Crown className="h-4 w-4 text-primary" />
                <div className="flex-1">
                  <p className="text-[11px] font-semibold uppercase tracking-widest text-primary/70">
                    Current Leader
                  </p>
                  <p className="text-sm font-bold text-foreground">{activeWinner.teamName}</p>
                </div>
                <div className="text-right">
                  <span className="font-mono text-sm">
                    <span className="font-semibold text-win">{activeWinner.matchWins}W</span>
                    <span className="text-muted-foreground/50">-</span>
                    <span className="text-loss">{activeWinner.matchLosses}L</span>
                  </span>
                </div>
              </div>
            )}
            ...
            <div className="mt-4 flex items-center gap-3">
              <Link
                href={`/competitions/${activeSeason.id}`}
        ...
        {seasons.length === 0 ? (
          <div className="rounded-lg border border-border bg-card p-12 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Competitions will appear here once configured.
            </p>
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {seasons.map((season) => {
              ...
              const winner = winnerBySeason.get(season.id)

              return (
                <Link
                  key={season.id}
                  href={`/competitions/${season.id}`}
                  className={cn(
                    "group flex flex-col rounded-xl border bg-card p-5 transition-all hover:shadow-md",
                    isActive
                      ? "border-primary/30 shadow-sm shadow-primary/5 hover:border-primary/50"
                ...
                >
                  ...
                  {/* Winner / Leader row */}
                  {winner && (
                    <div className="mb-4 flex items-center gap-2 rounded-lg bg-secondary/50 px-3 py-2.5">
                      <Crown className={cn(
                        "h-3.5 w-3.5",
                        isEnded ? "text-primary" : "text-muted-foreground",
                      )} />
                      ...
                      <span className="shrink-0 font-mono text-xs">
                        <span className="font-semibold text-win">{winner.matchWins}W</span>
                        <span className="text-muted-foreground/50">-</span>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {matchesBySeason.get(null)?.length} matches not assigned to any competition
                </p>
              </div>
              <Link
                href="/matches"
